...
**泛型类**，在书写的时候，在类名后面加 一对 **<>**；

尖括号里面用来指代一**个数据类型**，这个数据类型可以当做“**变量**”来使用，所谓的当做变量使用，是**指存储数据类型的变量**；
...
类似于方法中的变量参数，此时类型也定义成**参数形式**（可以称之为**类型形参**）

然后在使用/调用时传入具体的类型（**类型实参**）

> 数据类型的变量：在进行变量声明、方法定义返回值的时候，都需要用到 数据类型，可以使用 这个 “变量” 来指代数据类型

泛型可以用到三个地方：

- 类：public class Generic<E> {}
- 接口：public interface GenericInt<T> {}
- 方法：public static <S> ArrayList<S> addElement(S s1, S s2, S s3, S s4)
