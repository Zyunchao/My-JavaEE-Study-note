...
![Java 集合框架](F:\_2020-JavaEE-Study\_My Note\数据结构\img\Java集合框架.png)
...
- 不同点
  ...
  - 数组可以存基本数据类型和引用数据类型
    ...
    集合**只能存引用数据类型**,如果要存**基本数据类型**,需要存**对应的包装类**
...
- List 集合的特点
  ...
  > 由于 List 的元素在存储时互不干扰，没有什么依赖关系，自然可以重复
...
- 链表结构：**非连续**的空间 (head next) => (data next) => => (data ^)
...
- 初始化大小是 0
- 第一个元素**添加**进来后，长度变成 10
- 如果 10 个元素存满了，要扩容，扩原有的 50%， **1.5 倍**
