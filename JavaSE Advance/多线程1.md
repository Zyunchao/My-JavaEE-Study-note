 ...
 * 线程安全问题：
 * 期望：
 *  我们所书写的线程中的代码，我们所期望的是一个线程能够完成的执行完代码块中的代码，并对数据进行一次处理
 *  在线程一执行完后，无论第二次的执行权归属于哪个线程，上一次线程已经跑完了一次流程，对于数据的处理也是我们所期望的
 *  一个线程执行完整的一次流程
 *
 * 实际：
 *  如果线程1 在执行流中的执行权被 线程2 抢走，（比如：Thread.sleep）, 那么，线程2 就会执行且对数据处理
 *  在线程1 抢回执行权后，继续执行代码操作数据，由于多个线程间操作的是同一个数据，那么这个数据就可能是已经被 线程2 处理过了
 *  而这个数据，现在就不再是线程1 期望轨道中的原数据了
 *  同理，这个数据也不是线程2 执行期望轨道中的原数据
 *  这样一来，线程之间出现了交叉点，对同一个数据进行了多次处理
 *  这样超出了我们的期望，所以说，这样是不安全的
 *  故称为线程安全问题
 *
 * 为什么会出现？
 *  多线程操作共享数据，交叉操作
...
- 特点：
  ...
  - 默认情况是**打开**的，只要有一个线程进去执行代码了，锁就会**关闭**（自动关闭）
  - 当线程**执行完**出来了，锁才会**自动打开**
